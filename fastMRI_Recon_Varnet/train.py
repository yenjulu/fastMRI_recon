        ...
        for phase in phases:
            ...
            for i, (x, y, csm, mask) in enumerate(dataloaders[phase]):  
                ...
                with torch.set_grad_enabled(phase=='train'):
                    y_pred, y_unet = model(x, csm, mask)
                    y_pred_mag = torch.abs(y_pred)
                    y_unet_mag = torch.abs(y_unet)
                ...
                y_unet = y_unet_mag.detach().cpu().numpy() # np.abs(r2c(y_unet.detach().cpu().numpy(), axis=1))
            ...
            if args.write_image > 0 and (epoch % args.write_image == 0):
                writers[phase].add_figure('img', display_img(np.abs(r2c(x[-1].detach().cpu().numpy())), mask[-1].detach().cpu().numpy(), \
                    y[-1], y_pred[-1], y_unet[-1], epoch_score[val_score_name]), epoch)
