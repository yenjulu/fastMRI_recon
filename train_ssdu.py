    ...
    for epoch in range(start_epoch, epochs):
        for phase in phases: 
            ...
            for i, (y, ref_kspace_tensor, nw_input_tensor, sens_maps_tensor, trn_mask_tensor, loss_mask_tensor) in enumerate(tqdm(dataloaders[phase])):  
                ...
                x = np.abs(r2c(x.detach(), axis=1).cpu().numpy())
            ...
            if args.write_image > 0 and (epoch % args.write_image == 0):
                writers[phase].add_figure('img', display_img(x[-1], trn_mask_tensor[-1].detach().cpu().numpy(), loss_mask_tensor[-1].detach().cpu().numpy(), \
                    y[-1], y_pred[-1], y_resnet[-1], epoch_score[val_score_name]), epoch)
