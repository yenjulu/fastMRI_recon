...
def img_normalize(img):
    ...
    return img
#display =======================
def display_img_ssdu(x, trn_mask, loss_mask, y, y_pred, y_dn, score=None):
...
def display_img_modl(x, mask, y, y_pred, y_dn, score=None):
    fig = plt.figure(figsize=(15,10))
    ax1 = plt.subplot2grid((2,6), (0,0), colspan=2)
    ax2 = plt.subplot2grid((2,6), (0,2), colspan=2)
    ax3 = plt.subplot2grid((2,6), (0,4), colspan=2)
    ...
    ax4.imshow(y_pred, cmap='gray')
    ax4.set_title('reconstruction')
    ax5.imshow(y_dn, cmap='gray')
    ax5.set_title('recon_denoiser')    
    im6 = ax6.imshow(np.abs(y_pred-y), cmap='gray', vmin=np.abs(y).min(), vmax=np.abs(y).max())
    ax6.set_title('diff')
